...
def run_psi_step2(
    ...
    print(f"Created mapping from {len(h_to_k_map)} hashes to their double-blinded values")
    
    # Sample of our own double-blinded values for comparison
    our_k_values = list(h_to_k_map.values())
    if our_k_values:
        print(f"Sample of our double-blinded values: {our_k_values[:3]}")
    
    # Do a direct check to see if any values match
    # This helps verify if the problem is in the intersection calculation
    our_k_set = set(our_k_values)
    partner_k_set = set(k_values)
    direct_intersection = our_k_set.intersection(partner_k_set)
    print(f"Direct check: Found {len(direct_intersection)} matching values")
    if direct_intersection:
        print(f"Matching values: {list(direct_intersection)[:3]}")
...
def run_psi_step3(
    ...
    if partner_k_values:
        ...
        # Also output some statistics to help troubleshoot
        int_values = [v for v in partner_k_values if isinstance(v, int)]
        print(f"Partner k values type check: {len(int_values)}/{len(partner_k_values)} are integers")
        if len(int_values) > 0:
            print(f"Partner k values min: {min(int_values)}, max: {max(int_values)}")
    
    # Get our k values
    our_k_values = list(h_to_k_map.values())
    print(f"We have {len(our_k_values)} double-blinded values")
    
    # Show a sample of our k values
    if our_k_values:
        print(f"Our k values (sample): {our_k_values[:3]}")
        int_values = [v for v in our_k_values if isinstance(v, int)]
        print(f"Our k values type check: {len(int_values)}/{len(our_k_values)} are integers")
        if len(int_values) > 0:
            print(f"Our k values min: {min(int_values)}, max: {max(int_values)}")
    
    # Try a different approach - convert all values to strings for comparison
    partner_k_str = set(str(v) for v in partner_k_values)
    our_k_str = set(str(v) for v in our_k_values)
    str_intersection = partner_k_str.intersection(our_k_str)
    print(f"String-based comparison found {len(str_intersection)} matches")
    ...
    # If we didn't find any matches with standard comparison, but found some with string comparison
    if len(intersection_keys) == 0 and len(str_intersection) > 0:
        print("WARNING: Found matches with string comparison but not with standard comparison!")
        print("This indicates a data type issue in the k values")
        # Try to recover by using string intersection
        recovered_keys = set()
        for k_str in str_intersection:
            # Find the corresponding k value in our map
            for h, k in h_to_k_map.items():
                if str(k) == k_str:
                    recovered_keys.add(k)
                    break
        print(f"Recovered {len(recovered_keys)} keys from string comparison")
        intersection_keys = recovered_keys
